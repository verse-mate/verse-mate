...
export interface CoachMailer {
  sendEmail(data: {
    ...
    /** Where a reply goes when From must be the authenticated domain (6.3a). */
    replyTo?: { name: string; email: string };
    text: string;
    html?: string;
  }): Promise<CoachSendResult> | CoachSendResult | Promise<void> | void;
}

/** Mirrors the consumer's SendResult, kept local like the mailer contract. */
...
export class CoachService {
  ...
  async reviewReport(reportId: string) {
    ...
    return new CoachReviewService(this.db).review(reportId);
  }

  /** Correct one dimension of an undelivered report (5.7). */
  async correctDimension(input: {
    reportId: string;
    dimensionN: number;
    score: number | null;
    rationale: string;
