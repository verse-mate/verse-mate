...
export class DailyVerseRepository {
  ...
  /**
   * The active byline (verse-by-verse) explanation markdown for a chapter, in
   * the caller's language when one exists. Mirrors the language resolution in
   * BibleRepository.getExplanation — exact locale first (en-US), then the base
   * language (en), then whatever the default language offers — so the widget
   * summary follows the same content the reader would show.
   *
   * Returns null when the chapter has no generated byline yet; the caller then
   * omits the summary rather than failing the verse-of-the-day response.
   */
  async getBylineExplanation({
    ...
    const row = await this.db
      ...
      .select("explanations.explanation")
      .orderBy(
        sql`CASE
          WHEN LOWER(explanations.language_code) = ${normalized} THEN 0
          WHEN LOWER(explanations.language_code) = ${base} THEN 1
          ELSE 2
        END`,
      )
