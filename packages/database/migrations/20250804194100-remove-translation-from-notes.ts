...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    .dropIndex("notes_user_book_chapter_translation_idx")
    .execute();
  ...
  await db.schema
    .createIndex("notes_user_book_chapter_idx")
    .on("notes")
    .columns(["user_id", "book_name", "chapter_number"])
    .execute();
}

export async function down(db: Kysely<any>): Promise<void> {
  // Drop the new index
  await db.schema.dropIndex("notes_user_book_chapter_idx").execute();
